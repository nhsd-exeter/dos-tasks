...
@patch(f"{file_path}.common.process_file", return_value={"1": {"id": "00001", "name": "Mock Create ST", "nationalranking": 1, "action": "CREATE"}, "2": {"id": "00002", "name": "Mock Update ST", "nationalranking": 1, "action": "UPDATE"}, "3": {"id": "00003", "name": "Mock Delete ST", "nationalranking": 1, "action": "DELETE"}})
...
def test_request_success(mock_send_start_message, mock_cleanup,  mock_report_summary_count , mock_process_extracted_data, mock_process_file, mock_retrieve_file_from_bucket, mock_db_connection):
    ...
    assert result == "Service types execution successful"
...
def test_create_query():
    ...
    assert query == """
        insert into pathwaysdos.servicetypes (id, name, nationalranking, searchcapacitystatus, capacitymodel, capacityreset) values (%s, %s, %s, %s, %s, %s, %s)

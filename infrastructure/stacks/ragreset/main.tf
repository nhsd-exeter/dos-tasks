module "ragreset_lambda" {
  ...
  name               = "cron-ragreset"
  image_uri          = "${var.aws_lambda_ecr}/${var.project_group_short}/${var.project_name_short}/cron-ragreset:${var.image_version}"
...
resource "aws_cloudwatch_event_rule" "scheduled_event_rule" {
  name                = "${var.project_group_short}-${var.project_name_short}-ragreset-rule"
